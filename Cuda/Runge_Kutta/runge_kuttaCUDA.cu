...
void callRK(double & calcPerS, Child *generation, const cudaConstants* cConstant, GPUMem & gpuValues, double timeInitial) {
    ...
    //to track how many children to simulate
    //One will be subtracted from this each time a child's simulation is completed 
    int childrenToSim = cConstant->num_individuals;
